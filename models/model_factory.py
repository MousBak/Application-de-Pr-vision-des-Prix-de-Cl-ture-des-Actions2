    ...
    def predict(self, data: pd.Series) -> np.ndarray:
        ...
        try:
            ...
            if data.isna().all():
                ...
                # Faire les prédictions directement avec le modèle existant
            ...
            # Pour les prédictions sur les données de test
                ...
                # Faire les prédictions en une seule fois
            ...
            print(f"Erreur lors des prédictions ARIMA : {str(e)}")
            # En cas d'erreur, retourner des NaN
            return np.full(len(data), np.nan)
